defmodule ChatMarketWeb.MarkBlockController do
  ...
  def create(conn, %{"blocks_users_id" => blocks_users_id}) do
    ...
    {:ok, %BlocksUsers{} = updated_block_user} <- BlocksUsersManager.update(block_user, %{status: :available}) |> IO.inspect(label: "UPDATE BLOCK") do
      redirect(conn, to: Routes.available_block_path(conn, :index, changed_block_user: updated_block_user))
    end
  end
end
